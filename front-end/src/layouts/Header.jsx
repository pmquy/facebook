...
export default function () {
  ...
  return <div className="w-full flex justify-between gap-5 px-5 py-2 bg-surface text-onSurface shadow-xs">
    <div className="flex gap-5 items-center">
      <Link to={'/'}>
        <FaFacebook className="w-8 h-8 text-primary" />
      </Link>
      <TextField fullWidth variant="standard" placeholder="Tìm kiếm" slotProps={{
        input: {
    ...
    <div className="flex gap-1 items-center justify-end shrink-0">
      ...
      <Link to={'/users/' + user._id}>
        <img className={'w-8 h-8 overflow-hidden rounded-full object-cover'} src={user.avatar.url}></img>
      </Link>
    </div >
  </div >
}