  ...
  return <div className="flex flex-col gap-3">
    {post.options.map((e, i) => {
      ...
      return <div className="flex flex-col gap-2">
        <div key={i} className="p-3 bg-background rounded-lg relative flex justify-between gap-5 items-center overflow-hidden">
          <div className="absolute top-0 left-0 h-full bg-primary bg-opacity-30" style={{ width: percentage + '%' }}></div>
          <div className="flex gap-5 items-center">
            <Checkbox ref={checkRef} onChange={checked => handleVote(checked, i)} checked={checked} />
            <div className="break-all">{e.content}</div>
          </div>
    ...
    <div className="text-primary text-xss font-semibold">{total ? total + ' lượt bình chọn' : 'Chưa có bình chọn'}</div>
  </div>
}