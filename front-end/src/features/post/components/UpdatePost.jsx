...
export default function () {
  ...
  return <div className=''>
    ...
    <div ref={ref} className={`left-1/2 -translate-x-1/2 top-1/2 fixed z-10 ${open ? '-translate-y-1/2' : 'translate-y-[1000px]'} transition-all duration-500 card p-5 w-[90%] max-h-screen max-sm:w-screen overflow-y-auto max-w-[500px] flex flex-col gap-5 text-black`}>
      ...
      <div className="border-2 border-teal p-2 rounded-lg">
        <Textarea defaultValue={post.content} placeholder={'Viết nội dung'} className={'w-full'} autoFocus={true} ref={contentRef} />
        <Upload videos={videos} images={images} setImages={setImages} setVideos={setVideos} />
      </div>
      <Button onClick={handleUpdatePost} className={'m-auto btn-teal'}>Đăng</Button>
    </div>
